...
public static class G25DistanceEraEndpoints
{
    public static void MapG25DistanceEraEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        endpoints.MapGet("/", GetAll)
            ...
            .Produces<IReadOnlyList<GetG25DistanceEraContract.Response>>(StatusCodes.Status200OK);

        endpoints.MapGet("/{id:int}", GetById)
            .RequireAuthorization("EmailVerified")
            .RequireRateLimiting("authenticated")
            .Produces<GetG25DistanceEraContract.Response>(StatusCodes.Status200OK);

        endpoints.MapPost("/", Create)
            .RequireAuthorization("AdminOnly")
            .RequireRateLimiting("strict")
            .Produces<GetG25DistanceEraContract.Response>(StatusCodes.Status201Created);
