...
public class Auth0EmailVerifiedClaimsTests
{
    ...
    [Fact]
    public void GetJwtEmailVerifiedBoolean_MissingClaim_IsNull()
    {
        var p = new ClaimsPrincipal(new ClaimsIdentity([new Claim("sub", "x")]));
        Assert.Null(Auth0EmailVerifiedClaims.GetJwtEmailVerifiedBoolean(p));
    }
    ...
    [InlineData("email_verified", "false", false)]
    [InlineData("http://schemas.auth0.com/email_verified", "true", true)]
    public void GetJwtEmailVerifiedBoolean_FromClaims(string type, string value, bool expected)
    {
        var p = new ClaimsPrincipal(new ClaimsIdentity([new Claim(type, value)]));
        Assert.Equal(expected, Auth0EmailVerifiedClaims.GetJwtEmailVerifiedBoolean(p));
    }
}
