...
- **Entities (migration `AddAncestralPortraits`).** `AncestralPortraitSet` (one per order: the purchase `TransactionId` (unique → anti-replay/idempotent), `Status`, `Error`) + child `AncestralPortrait`s (per era × variation; `IsSelected` marks the user's pick — exactly one per era, default variation 0). Bytes are **PRIVATE** in R2 (`users/{identityId}/ancestral-portraits/{setId}/{eraId}-{i}.jpg`), served only via the authenticated `GET /ancestral-portraits/{id}/download` (`Results.File`) with an ownership check — **never `GetPublicUrl`** (images of the user's face). Delete/regenerate explicitly remove the R2 objects.
...
- **Cost is bounded by the entitlement** (`AncestralPortraitLimits`: `MaxEras`×`VariationsPerEra` images at a fixed size/quality, one set per order). Per-era upstream failures are caught (the set still succeeds with the eras that worked); `moderation_blocked` → the set's `Error`. Prompts are population-derived (no free-text user input) → low abuse/moderation risk.
- **Wiring.** `DbSet`s, `services.AddScoped<IAncestralPortraitService/Worker>`, `v1.MapAncestralPortraitEndpoints()`, `AncestralPortraitLimitsOptions` bound. **Needs `OpenAI:ApiKey` (already used by the admin image gen) + run `gen:api`.** The curated 29 per-population prompts still live in the FE manifest — v1 falls back to name+description server-side (a future improvement seeds them into `QpadmPopulation.ImagePrompt`).
