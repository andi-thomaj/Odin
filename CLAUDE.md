...
- **Re-buyable iterations (each purchase = a new kept set).** An order holds **MANY** sets — the user can buy "Ages" again and again, and every iteration is kept + listed. Endpoints: **`GET /orders/{orderId}/ancestral-portraits`** → ALL iterations newest-first (`ListSetsAsync`); **`POST …/purchase`** → creates a NEW set each genuine transaction (a *replayed* transaction is still idempotent via the unique `TransactionId`); **`GET /ancestral-portraits/sets/{setId}`** → poll one iteration; **`POST /ancestral-portraits/sets/{setId}/generate`** → retry one iteration. (The old single-set-per-order `GET`/`generate-by-orderId` are gone — `RequestGenerateAsync` now takes a `setId`.)
...
- **No duplicate portraits (load-bearing).** A doubled-portraits bug came from the generation job running twice (duplicate enqueue / concurrent worker). `RunGenerationAsync` now **atomically claims** the run with a conditional `ExecuteUpdateAsync` (`Pending`/`Failed` → `Running` only; a `Running`/`Succeeded` set returns `claimed == 0` and is skipped) so only one worker generates a set. Belt-and-braces: era groups are **de-duped by `EraId`** before generating, and `MapSet` collapses any duplicate variations (`GroupBy VariationIndex`) so already-affected data also renders clean.
