...
- **Reuses the admin image pipeline at the SDK seam.** `AncestralPortraitService.RunGenerationAsync` builds a prompt per era (`AncestralPortraitPrompts.Build` — identity-preserving lead "the SAME person … reimagined as <population>" + the curated `QpadmPopulation.ImagePrompt` or a name+description fallback + a photoreal style) and calls `IOpenAIImageClient.EditAsync` directly with the caller's `UserFacePhoto` bytes (`r2Storage.DownloadAsync`, ≤`MaxFaceReferences`) as `OpenAIReferenceImage`s — the SAME edit path `ImageGenerationService.RunEditAsync` uses, but the refs come from the user's face set (not admin `ReferenceImage`s) and there's no admin gate. Runs on a Hangfire worker (`IAncestralPortraitWorker`, `[Queue("default")]`/`[AutomaticRetry(Attempts=0)]` on the **interface**); the iOS client **polls** (no SignalR).
- **Entities (migration `AddAncestralPortraits`; index relaxed in `AllowMultipleAncestralPortraitSetsPerOrder`).** `AncestralPortraitSet` (purchase `TransactionId` unique → anti-replay; **`OrderId` index is NON-unique → many iterations per order**; `Status`, `Error`, `CreatedAt` orders the history) + child `AncestralPortrait`s (per era × variation; `IsSelected` toggled per portrait — any subset). Bytes are **PRIVATE** in R2 (`users/{identityId}/ancestral-portraits/{setId}/{eraId}-{i}.{ext}`, ext from the configured output format), served only via the authenticated `GET /ancestral-portraits/{id}/download` (`Results.File`) with an ownership check — **never `GetPublicUrl`** (images of the user's face). Delete/regenerate explicitly remove the R2 objects.
...
- **Generation is fully runtime-configurable from the web admin (migration `AddAncestralPortraitSettings`).** The model, size, quality, background, output format, moderation, `VariationsPerEra`, `MaxEras`, `MaxFaceReferences`, and the cost-estimate rates are a **single-row DB entity** `AncestralPortraitSettings` (`Id = 1`, `ancestral_portrait_settings`) behind `IAncestralPortraitSettingsService` — **the exact `ImageGenerationSettings`/`ImageSettingsService` pattern** (memory-cached, 5-min TTL, `UseCache => !Testing`, seeded from the entity's property defaults on first read, cache invalidated on write). The worker reads it per run via `GetAsync` (no redeploy to retune; **change the model from `gpt-image-2` here too**). Edit via **`GET`/`PUT /v1/api/admin/ancestral-portraits/settings`** (`AdminOnly`; `Request` validates against the shared `ImageParameterVocabulary` + `ImageRequestValidation.ValidateEnum`, caps `VariationsPerEra` 1–8 / `MaxEras` 1–20 / `MaxFaceReferences` 1–16, cost rates ≥ 0). The **old `AncestralPortraitLimitsOptions` is deleted** — its fields moved into this entity. The output format drives the R2 key extension + `ContentType` (`ResolveContentType` → jpeg/png/webp). Per-era upstream failures are caught (the set still succeeds with the eras that worked); `moderation_blocked` → the set's `Error`. Prompts are population-derived (no free-text user input) → low abuse/moderation risk.
